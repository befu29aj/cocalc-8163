...
\begin{equation}
  \label{eq:iepichae}
  \lim_{\vartheta\rightarrow{}0}\frac{\sin{}\vartheta}{\vartheta}
\end{equation}
...
\begin{frame}
  \frametitle{Squeeze Theorem}
\begin{figure}[h]
\includegraphics[scale=.3]{./limsinxoverx.png}
\end{figure}
\end{frame}
...
\begin{frame}
  ...
  $\bar{OP}=\bar{OR}=1$ and the angle $x$ at $O$. For simplicity let's
...
\begin{frame}
  ...
  \begin{equation}
    \label{eq:uraewiha}
    x\leq\bar{PS}+\bar{SR}\leq\bar{PS}+\bar{SQ}=\bar{PQ}=\tan{}x
  \end{equation}
$\bar{SR}\leq\bar{SQ}$ because the angle $QRS$ is a right angle.
